    ...
    <div ref="queryFrom" style="margin-bottom: 20px;">
      <el-card class="filter-container" shadow="never" body-style="padding: 5px 20px 0px 20px;">
        <div style="margin-bottom: 20px">
          ...
          <el-button v-if="isAuth('syslog:delete')" size="mini" type="danger" style="float:right;margin-right: 10px" @click="flushdb()"><svg-icon icon-class="clear" /> 清空redis数据</el-button>
            ...
            <el-row>
              <el-col :span="6">
                <el-form-item prop="username" >
                  <el-input v-model="dataForm.username" placeholder="key值" clearable>
                    <template slot="prepend">key值</template>
                  </el-input>
                </el-form-item>
              </el-col>
            </el-row>
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-button v-if="isAuth('syslog:delete')" type="text" size="small" class="red" @click.stop="deleteHandle(scope.row.key)"><svg-icon icon-class="delete-material" />删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
export default {
  ...
  data() {
    return {
      ...
      dataRule: {},
      queryUrl: '/redis/list',
      deleteUrl: '/redis/delete',
      exportUrl: '/redis/export',
      queryList: [],
      tableHight: 0
    }
  },
  ...
  methods: {
    ...
    flushdb() {
      this.$confirm(`确定清空操作?所有人的登录信息将被清空`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.$http.post(this.$http.adornUrl(`/redis/flushdb`)).then(({ data }) => {
    ...
    deleteHandle(id) {
      ...
      this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
        ...
        this.$http({
          url: this.$http.adornUrl('/redis/delete'),
          method: 'post',
